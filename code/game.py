...
from settings import SCREEN_HEIGHT, SCREEN_WIDTH, main_clock, WHITE, FPS
...
class Game(Enemy, Camera, Building):
    def __init__(self):
        ...
        self.sound = Sound()
        self.running = True
        self.end_screen_timer = 20
        self.add_coin_timer = 10
    ...
    def add_coin_every_sec(self, dt):
        self.add_coin_timer -= 10 * dt
        if self.add_coin_timer < 0:
            self.player_coins += random.randrange(1, 6)
            self.add_coin_timer = 30
    ...
    def draw_end_screen(self, fps):
        if fps == "ON":
            self.fps_txt = self.my_font.render("Fps:" + str(round(main_clock.get_fps(), 1)), False, WHITE)
            self.display_surface.blit(self.fps_txt, (0, 0))
        self.end_screen_timer -= 0.01
        if self.end_screen_timer > 0:
